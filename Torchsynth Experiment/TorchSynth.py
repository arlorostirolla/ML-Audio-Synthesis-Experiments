...
from torchsynth.module import (MonophonicKeyboard, SineVCO, SquareSawVCO, FmVCO,
                                ADSR,VCA,LFO,Noise)
...
class FlexibleSynthesizer(torch.nn.Module):
    def __init__(self, sample_rate: int = 44100, num_oscillators: int = 6, num_blend_methods: int = 3):
        super().__init__()
        self.sample_rate = sample_rate
        self.duration=5
        self.keyboard = MonophonicKeyboard(synthconfig)
        self.oscillators = torch.nn.ModuleList([
            SineVCO(synthconfig),
            SquareSawVCO(synthconfig),
            FmVCO(synthconfig),
            Noise(synthconfig, 0),
            WhiteNoiseOscillator(),
            PinkNoiseOscillator()
        ][:num_oscillators])
        self.envelope = ADSR(synthconfig)
        self.filter = SVFilter()
        ...
        self.filters = ["lowpass", "bandpass", "highpass"]

                # Define learnable parameters
        self.osc_type = torch.nn.Parameter(torch.randint(num_oscillators, size=(1,)).float())
        self.osc_type2 = torch.nn.Parameter(torch.randint(num_oscillators, size=(1,)).float())
        self.filter_type = torch.nn.Parameter(torch.randint(3, size=(1,)).float())
        self.blend_method = torch.nn.Parameter(torch.randint(num_blend_methods, size=(1,)).float())
    ...
    def forward(self, midi_f0, duration, note_on_duration, adsr_params, filter_params, osc_type, filter_type, osc_type2=None, blend_method=None):
        ...
        audio = self.filter(self.filters[filter_type])(audio, filter_params)
